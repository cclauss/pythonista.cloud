...
CLOUD_URL = 'http://forum.omz-software.com/topic/2775/cloud-import'

def Import(sTarget):
	soup = bs4.BeautifulSoup(urllib2.urlopen(CLOUD_URL).read())
	for code in soup.find_all('code', class_='xml'):
		s = code.getText()
		if s[:5] == '<?xml':
			#print(plistlib.readPlistFromString(s))
	...
	if iNow > iOld:
		console.hud_alert('updating {} ...'.format(sTarget))
