...
import bs4, urllib2, plistlib, shutil, zipfile, os, importlib, inspect, html2text, pickle, 
import bs4, console, html2text, importlib, inspect, os, pickle, plistlib, shutil, urllib2
			...
			for member in zipfile.ZipFile(f).namelist():
				...
				if len(l) <= 2: # module
					if l[-1][-3:] == '.py':
						...
						shutil.move(DOCS_DIR, member), os.path.join(SITE_DIR, l[-1]))
				else: # package
